...
`.trim()
                        if (user.common >= 1) {
                            ...
                            user.uncommon += cu * 1
                            user.common += cc * 1
                            await conn.reply(m.chat, Hcom, m)
                        } else await conn.reply(m.chat, 'Common crate anda tidak cukup', m)
                        break
                    case '10':
...
`.trim()
                        if (user.common >= 10) {
                            ...
                            user.uncommon += cu1 * 1
                            user.common += cc1 * 1
                            await conn.reply(m.chat, Hcom1, m)
                        } else await conn.reply(m.chat, 'Common crate anda tidak cukup', m)
                        break
                    case '100':
...
`.trim()
                        if (user.common >= 100) {
                            ...
                            user.uncommon += cu2 * 1
                            user.common += cc2 * 1
                            await conn.reply(m.chat, Hcom2, m)
                        } else await conn.reply(m.chat, 'Common crate anda tidak cukup', m)
                        break
                    case '1000':
...
`.trim()
                        if (user.common >= 1000) {
                            ...
                            user.uncommon += cu3 * 1
                            user.common += cc3 * 1
                            await conn.reply(m.chat, Hcom3, m)
                        } else await conn.reply(m.chat, 'Common crate anda tidak cukup', m)
                        break
                    default:
...
`.trim()
                        if (user.uncommon >= 1) {
                            ...
                            user.common += uc * 1
                            user.uncommon += uu * 1
                            await conn.reply(m.chat, Hun, m)
                            if (umc > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Rare yaitu*\n${umc} Mythic Crate`)
                                user.mythic += umc * 1
                            }
                        } else await conn.reply(m.chat, 'Uncommon crate anda tidak cukup', m)
                        break
                    case '10':
...
`.trim()
                        if (user.uncommon >= 10) {
                            ...
                            user.common += uc1 * 1
                            user.uncommon += uu1 * 1
                            await conn.reply(m.chat, Hun1, m)
                            if (umc1 > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Rare yaitu*\n${umc1} Mythic Crate`)
                                user.mythic += umc1 * 1
                            }
...
`.trim()
                        if (user.uncommon >= 100) {
                            ...
                            user.common += uc2 * 1
                            user.uncommon += uu2 * 1
                            await conn.reply(m.chat, Hun2, m)
                            if (umc2 > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Rare yaitu*\n${umc2} Mythic Crate`)
                                user.mythic += umc2 * 1
                            }
...
`.trim()
                        if (user.uncommon >= 1000) {
                            ...
                            user.common += uc3 * 1
                            user.uncommon += uu3 * 1
                            await conn.reply(m.chat, Hun3, m)
                            if (umc3 > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Rare yaitu*\n${umc3} Mythic Crate`)
                                user.mythic += umc3 * 1
                            }
...
`.trim()
                        if (user.mythic >= 1) {
                            ...
                            user.common += mc * 1
                            user.uncommon += mu * 1
                            await conn.reply(m.chat, Mychat, m)
                            if (mmm > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Rare yaitu*\n${mmm} Mythic Crate`)
                                user.mythic += mmm * 1
                            }
                            if (ml > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Epic yaitu*\n${ml} Legendary Crate`)
                                user.legendary += ml * 1
                            }
                        } else await conn.reply(m.chat, 'Mythic crate anda tidak cukup', m)
                        break
                    case '10':
...
`.trim()
                        if (user.mythic >= 10) {
                            ...
                            user.common += mc1 * 1
                            user.uncommon += mu1 * 1
                            await conn.reply(m.chat, Mychat1, m)
                            if (mmm1 > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Rare yaitu*\n${mmm1} Mythic Crate`)
                                user.mythic += mmm1 * 1
                            }
...
`.trim()
                        if (user.mythic >= 100) {
                            ...
                            user.common += mc2 * 1
                            user.uncommon += mu2 * 1
                            await conn.reply(m.chat, Mychat2, m)
                        } else await conn.reply(m.chat, 'Mythic crate anda tidak cukup', m)
                        break
                    case '1000':
...
`.trim()
                        if (user.mythic >= 1000) {
                            ...
                            user.common += mc3 * 1
                            user.uncommon += mu3 * 1
                            await conn.reply(m.chat, Mychat3, m)
                            if (mmm3 > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Rare yaitu*\n${mmm3} Mythic Crate`)
                                user.mythic += mmm3 * 1
                            }
...
`.trim()
                        if (user.legendary >= 1) {
                            ...
                            if (ll > 0 || lpp > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Epic yaitu*${ll > 0 ? `\n${ll} Legendary Crate` : ''}${lpp > 0 ? `\n${lpp} Pet Crate` : ''}`)
                                user.legendary += ll * 1
                                user.pet += lpp * 1
                            }
                        } else await conn.reply(m.chat, 'Legendary crate anda tidak cukup', m)
                        break
                    case '10':
...
`.trim()
                        if (user.legendary >= 10) {
                            ...
                            if (ll1 > 0 || lpp1 > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Epic yaitu*${ll1 > 0 ? `\n${ll1} Legendary Crate` : ''}${lpp1 > 0 ? `\n${lpp1} Pet Crate` : ''}`)
...
`.trim()
                        if (user.legendary >= 1000) {
                            ...
                            if (ll3 > 0 || lpp3 > 0) {
                                await m.reply(`*Selamat anda mendapatkan item Epic yaitu*${ll3 > 0 ? `\n${ll3} Legendary Crate` : ''}${lpp3 > 0 ? `\n${lpp3} Pet Crate` : ''}`)
            ...
            case 'pet':
                ...
                if (user.pet > 0) {
                    ...
                    if (_pet == 'kucing' && kucing > 0) {
                        ...
                        await conn.reply(m.chat, `Anda sudah memiliki pet ${_pet}, Hadiahmu diganti dengan 2 potion${mknp > 0 ? ` Dan ${mknp} Makanan Pet` : ''}`, m)
                    } else if (_pet == 'kucing' && kucing == 0) {
                        user.kucing += 1
                        user.makananpet += mknp * 1
                        await conn.reply(m.chat, `*Selamat Anda mendapatkan pet${_pet} ${mknp > 0 ? ` Dan ${mknp} Makanan Pet*` : '*'}`, m)
                        ...
                        user.makananpet += mknp * 1
                        await m.reply(pickRandom(['Anda kurang beruntung', 'Coba buka lagi lain kali, karena gk dapet pet', 'kasian gk dapet pet', 'Mungkin lagi gk hoki dan gk dapet pet', 'wkwkkwkwke']) + '. Anda hanya mendapatkan *' + mknp + '* makanan pet')
                    }
                } else await m.reply('Pet Crate kamu tidak cukup')
                break
            default:
