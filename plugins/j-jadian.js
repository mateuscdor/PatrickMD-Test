    ...
    try {
        ...
        if (global.db.data.users[m.sender].pasangan != "" && global.db.data.users[global.db.data.users[m.sender].pasangan].pasangan == m.sender && global.db.data.users[m.sender].pasangan != user) {
            ...
            if (global.db.data.users[pacar].pasangan == user) {
                if (m.sender == pacar && global.db.data.users[m.sender].pasangan == user) return conn.reply(m.chat, `Anda sudah berpacaran dengan @${user.split('@')[0]}`, m, {
                    //contextInfo: {
                    mentions: [user]
                    //}
                })
                ...
                conn.reply(m.chat, `Anda baru saja mengajak @${user.split('@')[0]} berpacaran`, m, { mentions: [user] })
                conn.sendButton(user, `${m.sender.split('@')[0]} baru saja mengajak anda berpacaran\n\nKetik *${usedPrefix}terima @user* untuk menerima\n*${usedPrefix}tolak @user untuk menolak*`, wm, null, [['Terima', `.terima ${m.sender.split('@')[0]}`], ['Tolak', `.tolak ${m.sender.split('@')[0]}`]], m, {
                    //contextInfo: {
                    mentions: [m.sender]
                    //}
                })
