...
Bonus: ${poin} XP
    ...
    conn.tebakbendera[id] = [
        await conn.sendButton(m.chat, caption, wm, await (await fetch(json.img)).buffer(), ['Bantuan', '.teben'], m)
        ,
        json, poin,
        setTimeout(async () => {
            if (conn.tebakbendera[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah *${json.name}*`, wm, null, ['Tebak Bendera', '.tebakbendera'], conn.tebakbendera[id][0])
