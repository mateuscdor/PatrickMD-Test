...
`.trim()
                ...
                if (mythic > 0) {
                    ...
                    await conn.reply(m.chat, '*Selamat anda mendapatkan item Rare yaitu*\n' + mythic + ' 🗳️ Mythic Crate', m)
                }
                if (legendary > 0) {
                    global.db.data.users[m.sender].legendary += legendary * 1
                    await conn.reply(m.chat, '*Selamat anda mendapatkan item Epic yaitu*\n' + legendary + ' 🗃️ Legendary Crate', m)
                }
            ...
            } else await conn.reply(m.chat, `Anda sudah berpetualang dan kelelahan, silahkan coba *${timers}* lagi`, m)
        } else await conn.reply(m.chat, 'Minimal 80 health untuk bisa berpetualang, beli nyawa dulu dengan ketik *' + usedPrefix + 'shop buy potion <jumlah>*\ndan ketik *' + usedPrefix + 'use potion <jumlah>*\n\n_Untuk mendapat money dan potion gratis ketik_ *' + usedPrefix + 'claim*', m)
    } catch (e) {
        throw eror
        //console.log(e)
