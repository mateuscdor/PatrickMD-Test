...
`.trim()
    ...
    conn.math[id] = [
        ...
        math, 4,
        setTimeout(async () => {
            if (conn.math[id]) await conn.sendButton(m.chat, `Waktu habis!\nJawabannya adalah ${math.result}`, wm, null, [`${math.mode.toUpperCase()}`, `.math ${math.mode}`], conn.math[id][0])
            delete conn.math[id]
        }, math.time)
    ]
}
...
let modes = {
    ...
    impossible2: [-999999999999999, 999999999999999, -999, 999, '/', 40000, 50000]
}
