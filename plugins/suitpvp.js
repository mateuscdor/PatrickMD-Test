...
let handler = async (m, { conn, usedPrefix }) => {
    ...
    if (m.mentionedJid[0] === m.sender) return m.reply(m.chat, `_*Tidak bisa bermain dengan diri sendiri.*_`)
    if (m.mentionedJid[0] === conn.user.jid) return m.reply(m.chat, `_*Tidak bisa bermain dengan saya. :')*_`)
...
`.trim()
    ...
    conn.suit[id] = {
        chat: await conn.sendButton(m.chat, caption, footer, null, [['Terima', 'ok'], ['Tolak', 'tolak']], m, {
            //contextInfo: {
