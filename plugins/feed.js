let handler = async (m, { conn, args, usedPrefix }) => {
    ...
    switch (type) {
        case 'rubah':
            if (rubah == 0) return await m.reply('*Kamu belum memiliki Pet Rubah*')
            if (rubah == 5) return await m.reply('*Pet kamu sudah lvl max*')
            ...
            if (new Date - global.db.data.users[m.sender].rubahlastclaim > 600000) {
                if (global.db.data.users[m.sender].makananpet > 0) {
                    ...
                    await conn.reply(m.chat, `Berhasil memberi makan pet ${type}`, m)
                    if (rubah > 0) {
                        ...
                        if (global.db.data.users[m.sender].anakrubah > naiklvl) {
                            ...
                            await conn.reply(m.chat, `*Selamat Pet Rubah kamu naik level*`, m)
                        }
                    }
                } else await m.reply(`Makanan pet kamu tidak cukup`)
            } else await m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktur}* lagi`)
            break
        case 'kuda':
            if (kuda == 0) return await m.reply('*Kamu belum memiliki Pet Kuda*')
            if (kuda == 5) return await m.reply('*Pet kamu dah lvl max*')
                ...
                if (global.db.data.users[m.sender].makananpet > 0) {
                    ...
                    if (kuda > 0) {
                        ...
                        if (global.db.data.users[m.sender].anakkuda > naiklvl) {
                            ...
                            await conn.reply(m.chat, `*Selamat Pet Kuda kamu naik level*`, m)
                        }
                    }
                } else await m.reply(`Makanan pet kamu tidak cukup`)
            } else await m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktuk}* lagi`)
            break
        case 'kucing':
            if (kucing == 0) return await m.reply('*Kamu belum memiliki Pet Kucing*')
            if (kucing == 5) return await m.reply('*Pet kamu sudah lvl max*')
            ...
            } else await m.reply(`Pet kamu sudah kenyang, coba kasih makan *${waktu}* lagi`)
            break
        default:
            return await conn.reply(m.chat, `${usedPrefix}feed [kucing | rubah | kuda]\n\nContoh penggunaan: *${usedPrefix}feed kucing*`, m)
    }
}
